...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!ARK_API_KEY || !ARK_MODEL) {
    return res.status(500).json({
      error: "Missing env",
      detail: "请在 Vercel Environment Variables 配置 ARK_API_KEY 与 ARK_MODEL",
    });
  }
  ...
  try {
    ...
    if (e?.message === "REQUEST_TOO_LARGE") {
      return res.status(413).json({
        ...
        detail:
          "你传到 /api/analyze 的图片 base64 过大，已超过 Vercel Function 4.5MB 限制。请在前端压缩/降分辨率后再传。",
      });
    }
  ...
  // 这里再做一次强校验：建议把图片压到 1.5MB 以内更稳
  const imgBytes = base64Bytes(imageBase64);
  if (imgBytes > 1.8 * 1024 * 1024) {
    return res.status(413).json({
      error: "Image too large",
      detail: `图片过大（约 ${(imgBytes / 1024 / 1024).toFixed(2)}MB）。请把前端截图分辨率/质量再降一点（建议 <= 1.5MB）。`,
    });
  }
