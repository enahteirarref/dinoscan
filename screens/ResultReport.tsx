        ...
        contents: [
          {
            role: 'user',
            parts: [
              { text: `请为恐龙 "${fossil.name}" 提供详细的古生物学简报。
              要求包含:
              1. 科学综述 (2-3句话)。
              2. 典型的栖息地描述。
              3. 3个独特且有趣的冷知识。
              ...
              务必使用简体中文。返回 JSON 格式: { "scientificOverview": "...", "habitat": "...", "funFacts": ["...", "...", "..."], "diet": "...", "youtubeQuery": "..." }` }
            ]
          }
        ],
        config: {
  ...
  return (
    <div className="h-screen w-full bg-background-light dark:bg-background-dark overflow-y-auto no-scrollbar pb-32 relative">
      <header className="sticky top-0 z-30 bg-background-light/80 dark:bg-gray-900/80 backdrop-blur-md px-6 py-4 flex justify-between items-center">
        <button onClick={onBack} className="w-10 h-10 bg-white dark:bg-gray-800 rounded-full shadow-sm flex items-center justify-center active:scale-90 transition-transform">
          <span className="material-symbols-outlined dark:text-white">arrow_back</span>
        </button>
        <h1 className="text-sm font-bold text-forest-green dark:text-primary uppercase tracking-widest">化石分析报告</h1>
      ...
      <main className="px-6 pt-2">
        <div className="flex flex-col items-center text-center mb-6">
          <div className="relative mb-1">
            <span className="text-4xl font-extrabold text-primary tracking-tighter">{fossil.matchConfidence}% 匹配度</span>
        ...
        <div className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden shadow-2xl ring-4 ring-white dark:ring-gray-800 mb-8 group">
          <img src={fossil.imageUrl} className="w-full h-full object-cover grayscale-[10%] group-hover:scale-110 transition-transform duration-1000" alt="Result" />
          ...
          <div className="absolute bottom-6 left-6 right-6">
            <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-md p-5 rounded-3xl flex justify-between items-center shadow-lg border border-white/40 dark:border-gray-700">
              <div>
                <h2 className="text-2xl font-black text-forest-green dark:text-white leading-none mb-1 uppercase tracking-tight">{fossil.name}</h2>
                <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em]">{fossil.classification}</p>
              </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-8">
          ...
          <StatCard icon="category" label="分类" value={fossil.classification} color="bg-green-50" iconColor="text-primary" />
          <StatCard icon="straighten" label="预估长度" value={fossil.length} color="bg-gray-100" />
          <StatCard icon="stars" label="稀有程度" value={fossil.rarity} color="bg-yellow-50" iconColor="text-yellow-600" isFilled />
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[2rem] p-6 shadow-sm mb-12 border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-sm font-black text-forest-green dark:text-white uppercase tracking-widest">现场观察笔记</h3>
          </div>
      ...
      <footer className="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-6 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-t border-gray-100 dark:border-gray-800 flex gap-4 z-40">
        <button onClick={handleLearnMore} className="flex-1 bg-gray-100 dark:bg-gray-800 py-4 rounded-2xl font-black text-forest-green dark:text-white text-xs uppercase tracking-widest flex items-center justify-center gap-2 active:scale-95 transition-all">
          <span className="material-symbols-outlined text-xl">school</span>
          深入研究
        </button>
        <button onClick={onSave} className="flex-[1.5] bg-primary py-4 rounded-2xl font-black text-white text-xs uppercase tracking-widest shadow-lg shadow-primary/30 flex items-center justify-center gap-2 active:scale-95 transition-all">
          ...
          <div className="w-full max-w-md bg-white dark:bg-gray-900 rounded-t-[3rem] shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto no-scrollbar pb-12">
            <div className="sticky top-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center z-10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-black text-forest-green dark:text-white uppercase tracking-widest">AI 研究简报</h3>
                  <p className="text-[10px] text-gray-400 font-bold uppercase">{fossil.name}</p>
                </div>
              </div>
              <button onClick={() => setShowModal(false)} className="w-10 h-10 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center active:scale-90 transition-transform">
            ...
            <div className="p-6">
              {isResearching ? (
                <div className="py-20 flex flex-col items-center justify-center text-center space-y-6">
                  ...
                  <p className="text-forest-green dark:text-primary font-black uppercase tracking-[0.2em] text-xs">正在调取古生物数据库...</p>
                </div>
              ) : researchData ? (
                <div className="space-y-8 animate-fadeIn">
                  <section>
                    <h4 className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3">科学综述</h4>
                    <p className="text-gray-600 dark:text-gray-400 text-sm leading-relaxed font-medium">{researchData.scientificOverview}</p>
                  </section>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">食性</p>
                      <p className="text-sm font-bold text-forest-green dark:text-white flex items-center gap-2">{researchData.diet}</p>
                    </div>
                    <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                      <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest mb-1">栖息地</p>
                      <p className="text-sm font-bold text-forest-green dark:text-white flex items-center gap-2">{researchData.habitat}</p>
                    </div>
                  </div>
                  <section>
                    <h4 className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-3">你不知道的冷知识</h4>
                    <ul className="space-y-3">
                      {researchData.funFacts.map((fact, i) => (
                        <li key={i} className="flex gap-3 text-sm text-gray-600 dark:text-gray-400 font-medium">
                          <span className="w-5 h-5 bg-primary/10 rounded-full flex items-center justify-center text-primary text-[10px] shrink-0 font-black">{i+1}</span>
                          {fact}
                        </li>
                      ))}
                    </ul>
...
const StatCard = ({ icon, label, value, color, iconColor = "text-forest-green", isFilled = false }: any) => (
  <div className={`rounded-3xl bg-white dark:bg-gray-800 p-4 shadow-sm border border-gray-50 dark:border-gray-700 flex flex-col gap-3 transition-all hover:-translate-y-1`}>
