...
const HomeDashboard: React.FC<HomeDashboardProps> = ({ onScanClick, onNavigate, currentScreen, onFossilClick }) => {
  ...
  const handleGeologySync = async () => {
    setLoading(true);
    setInsight({ title: "正在获取地理坐标...", content: "正在通过卫星定位您当前的岩层位置并查询地质数据库..." });
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `你是一位地质学和古生物学专家。当前坐标为：纬度 ${latitude.toFixed(4)}, 经度 ${longitude.toFixed(4)}。
        请结合此位置的实时地理信息，分析其地表下是否存在沉积岩（如砂岩、泥岩），并判断其所属的地质年代（如白垩纪、侏罗纪）。
        最后给出在该处发现恐龙化石的可能性评估。
        请务必使用专业且亲切的简体中文回答。
        返回 JSON 格式: { "title": "地层综合评估", "content": "详细的分析内容..." }`,
        config: { 
      ...
      setInsight({ title: "同步失败", content: "无法获取您的 GPS 信号或地层数据库连接超时。" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleInsightClick = async (topic: string) => {
    setLoading(true);
    setInsight({ title: "正在同步存档...", content: `正在查询关于 ${topic} 的真实科研数据...` });
    
    try {
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `你是一位古生物学家。请针对 "${topic}" 提供一段真实的、温暖的科学简报。请确保信息基于真实的科学发现。必须使用简体中文。返回 JSON 格式: { "title": "...", "content": "..." }`,
        config: { 
          responseMimeType: "application/json",
          tools: [{ googleSearch: {} }]
        }
      ...
      setInsight({ title: "同步中断", content: "归档连接失败。" });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-background-light dark:bg-background-dark transition-colors duration-500">
      <header className="px-6 pt-12 pb-6 flex justify-between items-end bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 shadow-sm">
        <div>
          <h1 className="text-3xl font-black text-forest-green dark:text-primary tracking-tighter leading-none">探寻<span className="text-primary dark:text-white">助手</span></h1>
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">野外科研协作平台 v3.6</p>
        </div>
      </header>
        ...
        <section className="mb-8">
          ...
          >
            ...
            <div className="relative z-10">
              <span className="bg-primary/30 text-primary px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-primary/40">实时 GPS 地层同步</span>
              <h2 className="text-3xl font-bold leading-tight mt-6">地层与岩性 <br/>深度探测</h2>
              <p className="text-white/60 text-xs mt-4 leading-relaxed max-w-[80%]">点击此处，系统将根据您当前坐标分析岩石属性及化石蕴藏潜力。</p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <InsightButton 
              icon="biotech" 
              label="生物矿化" 
              onClick={() => handleInsightClick("生物矿化")} 
            />
            <InsightButton 
            ...
            <InsightButton 
              icon="monitoring" 
              label="激光雷达" 
              onClick={() => handleInsightClick("激光雷达探测")} 
            />
          </div>
        </section>
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white dark:bg-gray-900 w-full max-w-md rounded-t-[3rem] shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto no-scrollbar pb-12">
            <div className="sticky top-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center z-10">
              ...
              <button onClick={() => setInsight(null)} className="w-10 h-10 bg-gray-100 dark:bg-gray-800 rounded-full flex items-center justify-center active:scale-90 transition-transform">
            ...
            <div className="p-8">
              {loading ? (
                <div className="py-20 flex flex-col items-center gap-6 text-center">
                  ...
                  <div>
                    <p className="text-xs font-black text-forest-green dark:text-primary uppercase tracking-widest">{insight.title}</p>
...
const InsightButton = ({ icon, label, onClick }: any) => (
  ...
  >
    ...
    <span className="text-[10px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest text-center">{label}</span>
  </button>
);
